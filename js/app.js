...
import { 
    ST, save, load, updateStarDisplay, say, showToast, earn, 
    resetProgress, showResult, VALID_CODES, sWin, sBad, 
    setResultNext 
} from './utils.js';
...
function ob_setAge(age, btn) { 
    ob_age = age; 
    document.querySelectorAll('#ob-2 .ob-choice').forEach(b => b.classList.remove('selected')); 
    btn.classList.add('selected') 
}

function ob_setLevel(level, btn) { 
    ob_level = level; 
    document.querySelectorAll('#ob-3 .ob-choice').forEach(b => b.classList.remove('selected')); 
    btn.classList.add('selected') 
}
...
function requirePremium(screenId) { 
    if (ST.premium) { goScreen(screenId); return } 
    showPremiumGate(screenId) 
}

function showPremiumGate(screenId) { 
    _pendingScreen = screenId || null; 
    document.getElementById('premium-gate').classList.add('show'); 
    document.getElementById('pg-code-msg').textContent = ''; 
    document.getElementById('pg-code-input').value = '' 
}

function closePremiumGate() { 
    document.getElementById('premium-gate').classList.remove('show'); 
    _pendingScreen = null 
}

function redeemCode() {
  const code = document.getElementById('pg-code-input').value.trim().toUpperCase(); 
  const msg = document.getElementById('pg-code-msg');
  if (!code) { msg.style.color = '#dc2626'; msg.textContent = 'Enter a code first!'; return }
  if (VALID_CODES.includes(code)) { 
      ST.premium = true; save(); 
      msg.style.color = '#16a34a'; msg.textContent = '✅ Premium unlocked!'; 
      import('./utils.js').then(u => u.confetti(80)); 
      sWin(); 
      setTimeout(() => { 
          closePremiumGate(); updatePremiumUI(); 
          if (_pendingScreen) goScreen(_pendingScreen) 
      }, 1200) 
  }
...
function goScreen(id) {
  ...
  if (id === 's-capture') initCapture(); 
  if (id === 's-defend') initDefend(); 
  if (id === 's-curriculum') buildCurriculum();
  if (id === 's-progress') renderProgress(); 
  if (id === 's-blunder') initBlunder(); 
  if (id === 's-patterns') initPatterns();
  if (id === 's-combo') initCombo(); 
  if (id === 's-endcoach') initEndCoach(); 
  if (id === 's-opening') initOpening();
  if (id === 's-eval') initEval(); 
  if (id === 's-pawn') initPawn(); 
  if (id === 's-defend-check') initDefendCheck();
  if (id === 's-ladder-method') initLadder(); 
  if (id === 's-fork-train') initForkTrain();
...
window.nextThreat = nextThreat;
window.showThreatHint = showThreatHint;
window.newDojoChallenge = newDojoChallenge;
window.nextCapture = nextCapture;
window.startDefend = startDefend;
window.showDefendHint = showDefendHint;
window.resetDefend = resetDefend;
window.blunderAnswer = blunderAnswer;
window.nextBlunder = nextBlunder;
window.blunderHint = blunderHint;
window.nextPatternChallenge = nextPatternChallenge;
window.closePat = closePat;
window.nextCombo = nextCombo;
window.comboHint = comboHint;
window.startEndgame = startEndgame;
window.endgameNext = endgameNext;
window.endgameHint = endgameHint;
window.resetEndgame = resetEndgame;
window.nextOpening = nextOpening;
window.evalAnswer = evalAnswer;
window.nextEval = nextEval;
window.evalHint = evalHint;
window.nextPawn = nextPawn;
window.pawnHint = pawnHint;
window.dkSetSkill = dkSetSkill;
window.dkNext = dkNext;
window.dkHint = dkHint;
window.ldSetMode = ldSetMode;
window.ldNext = ldNext;
window.ldHint = ldHint;
window.ldReset = ldReset;
window.ftSetSkill = ftSetSkill;
window.ftNext = ftNext;
window.ftHint = ftHint;
window.setDiff = setDiff;
window.startBotGame = startBotGame;
window.showBotHint = showBotHint;
window.resetBotGame = resetBotGame;
window.closeResult = (again) => { 
    document.getElementById('rover').classList.remove('show'); 
    import('./utils.js').then(u => {
        if (again && u.resultNext) u.resultNext(); 
