...
import { getEvals } from '../data_loader.js';

export let EV={ idx:0,correct:0,total:0,answered:false };
let EVALS=[];

export async function initEval() {
  EVALS=await getEvals();
  EV={ idx:Math.floor(Math.random()*EVALS.length),correct:0,total:0,answered:false }; loadEval();
}
...
export function loadEval() {
  ...
  hideFB('ev-fb'); document.getElementById('ev-next').style.display='none';
...
export function evalAnswer(ans) {
  if(EV.answered)return; sTick(); EV.answered=true; EV.total++;
  const e=EVALS[EV.idx]; const correct=ans===e.ans; if(correct)EV.correct++;
