...
import { PAWNS, PWCATS } from '../data.js';

export let PW = { cat: 'all', idx: 0, answered: false }; 
let pwBuilt = false;

export function initPawn() { if (!pwBuilt) { pwBuilt = true; buildPwCats() } loadPawn() }
...
export function buildPwCats() { 
    ...
    PWCATS.forEach(cat => { 
        ...
        b.onclick = () => { 
            document.querySelectorAll('#s-pawn .pcat').forEach(x => x.classList.remove('on')); 
            b.classList.add('on'); PW.cat = cat.id; PW.idx = 0; loadPawn() 
        }; nav.appendChild(b) 
    }) 
}
...
export function loadPawn() { 
    const list = getPwList(); if (!list.length) return; 
    const pw = list[PW.idx % list.length]; PW.answered = false; 
    ...
    hideFB('pw-fb'); 
    document.getElementById('pw-next').style.display = 'none'; 
    ...
    if (pw.type === 'spot') { drawBoard('pw-board', pw.fen, { click: (sn) => handlePawnSpot(sn, pw) }) } 
        ...
        if (pw.q) { 
            const q = document.createElement('div'); q.style.cssText = 'font-weight:800;font-size:.88rem;margin-bottom:8px;padding:9px 12px;background:var(--card);border:2.5px solid var(--ink);border-radius:var(--r-sm)'; q.textContent = pw.q; ch.appendChild(q) 
        } 
        ...
        ['Yes ✅', 'No ❌'].forEach((lbl, i) => { 
            ...
            b.onclick = () => { 
                if (PW.answered) return; PW.answered = true; sTick(); 
                const ok = (i === 0) === pw.ya; 
                if (ok) { sOk(); confetti(25); earn(2, 'pawn'); showFB('pw-fb', '✅ ' + pw.exp, 'ok'); b.className = 'yn-btn ok' } 
                else { sBad(); showFB('pw-fb', '❌ ' + pw.exp, 'no'); b.className = 'yn-btn wrong' }; 
                document.getElementById('pw-next').style.display = ''; say(pw.exp) 
            }; yn.appendChild(b) 
        }); 
        ch.appendChild(yn) 
    } 
...
export function handlePawnSpot(sn, pw) { 
    ...
    if (pw.ans.includes(sn)) { 
        sOk(); confetti(25); earn(2, 'pawn'); 
        showFB('pw-fb', '✅ ' + pw.exp, 'ok'); drawBoard('pw-board', pw.fen, { g: [sn] }) 
