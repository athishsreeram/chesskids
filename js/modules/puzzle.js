...
import { getPuzzles,getPcats } from '../data_loader.js';

export let puzState={ cat:'all',idx:0,sel:null,solved:false,board:null };
let PUZZLES=[],PCATS=[];

export async function initPuzzle() {
  PUZZLES=await getPuzzles(); PCATS=await getPcats();
  const nav=document.getElementById('puz-cats'); if(nav.children.length>0){loadPuzzle();return;}
