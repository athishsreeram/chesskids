...
export function initForkTrain() { ftSetSkill('fork') }

export function ftSetSkill(skill) { 
    FT = { skill, idx: 0, sel: null, answered: false };
    ['fork', 'double'].forEach(s => { 
        const btn = document.getElementById('ft-tab-' + s); if (btn) btn.classList.toggle('on', s === skill) 
    }); 
...
export function ftLoad() { 
    const list = FT_SCENARIOS[FT.skill]; 
    const sc = list[FT.idx % list.length]; 
    FT.sel = null; FT.answered = false; 
    document.getElementById('ft-ctr').textContent = 'Scenario ' + (FT.idx % list.length + 1) + '/' + list.length; 
    document.getElementById('ft-kip').textContent = sc.task; hideFB('ft-fb'); 
    document.getElementById('ft-next').style.display = 'none'; 
    const div = document.getElementById('ft-targets'); div.innerHTML = ''; 
    if (sc.pairs) { 
        sc.pairs.forEach(([code]) => { 
            const tag = document.createElement('div'); tag.className = 'fork-target'; tag.id = 'ft-tgt-' + code; tag.textContent = PM[code] || code; div.appendChild(tag) 
        }) 
    }; 
    say(sc.task); 
    drawBoard('ft-board', sc.fen, { click: (sn, r, f) => handleFork(sn, r, f, sc) }) 
}
...
export function handleFork(sn, r, f, sc) { 
    ...
    if (!FT.sel) { 
        if (p && p[0] === 'w') { 
            ...
            drawBoard('ft-board', sc.fen, { sel: sn, dot: lm, click: (s2, r2, f2) => handleFork(s2, r2, f2, sc) }) 
        } 
        return 
    } 
    if (sn === FT.sel) { 
        FT.sel = null; 
        drawBoard('ft-board', sc.fen, { click: (s2, r2, f2) => handleFork(s2, r2, f2, sc) }); return 
    } 
    const { r: fr, f: ff } = srf(FT.sel); 
    const legal = getLegalMovesFiltered(bd, fr, ff); 
    if (!legal.includes(sn)) { 
        if (p && p[0] === 'w') { 
            FT.sel = sn; 
            drawBoard('ft-board', sc.fen, { sel: sn, dot: getLegalMovesFiltered(bd, r, f), click: (s2, r2, f2) => handleFork(s2, r2, f2, sc) }); return 
        } 
        showFB('ft-fb', 'Cannot! ' + sc.hint, 'no'); sBad(); FT.sel = null; 
        drawBoard('ft-board', sc.fen, { click: (s2, r2, f2) => handleFork(s2, r2, f2, sc) }); return 
    } 
    if (sc.answers.includes(sn)) { 
        FT.answered = true; const nb = applyMove(bd, fr, ff, r, f); 
        drawBoard('ft-board', nb, { g: [sn] }); 
        if (sc.pairs) sc.pairs.forEach(([code]) => { const el = document.getElementById('ft-tgt-' + code); if (el) el.classList.add('found') }); 
        showFB('ft-fb', '🍴 ' + sc.explain, 'ok'); sOk(); confetti(50); earn(3, 'fork'); 
        document.getElementById('ft-next').style.display = ''; say(sc.explain) 
    } else { 
        showFB('ft-fb', '❌ Not two pieces! ' + sc.hint, 'no'); sBad(); FT.sel = null; 
        drawBoard('ft-board', sc.fen, { click: (sn2, r2, f2) => handleFork(sn2, r2, f2, sc) }) 
    } 
}
