...
import { getDojoPieces } from '../data_loader.js';
...
export async function initDojo() {
  DOJO_PIECES=await getDojoPieces();
  const btns=document.getElementById('dojo-piece-btns'); if(btns.children.length>0) return;
  DOJO_PIECES.forEach(p=>{ const b=document.createElement('button'); b.className='dojo-piece-btn'; b.textContent=p.icon; b.title=p.label; b.onclick=()=>selectDojoPiece(p,b); btns.appendChild(b); });
}

export function selectDojoPiece(p,btn) {
  sTick(); document.querySelectorAll('.dojo-piece-btn').forEach(b=>b.classList.remove('on')); btn.classList.add('on');
  dojoState.piece=p; dojoState.moves=0; dojoState.trail=[]; dojoState.sel=null;
  document.getElementById('dojo-msg').textContent=p.fact+' Navigate to the 🌟 target!';
  say(p.fact); hideFeedback('dojo-feedback'); newDojoChallenge();
}

export function newDojoChallenge() {
  if(!dojoState.piece) return;
  let fr,ff,tr,tf; do{fr=Math.random()*8|0;ff=Math.random()*8|0;tr=Math.random()*8|0;tf=Math.random()*8|0;}while(fr===tr&&ff===tf);
  dojoState.from={r:fr,f:ff}; dojoState.target={r:tr,f:tf}; dojoState.moves=0; dojoState.trail=[]; dojoState.sel=null;
  dojoState.best=calcMinMoves(dojoState.piece.code,fr,ff,tr,tf); updateDojoStats(); renderDojo();
}

export function calcMinMoves(code,fr,ff,tr,tf) {
  const visited=new Set(); const queue=[{r:fr,f:ff,moves:0}]; visited.add(fr+','+ff);
  while(queue.length){const{r,f,moves}=queue.shift();if(r===tr&&f===tf)return moves;const tmpBd=Array.from({length:8},()=>Array(8).fill(null));tmpBd[r][f]=code;getLegalMoves(tmpBd,r,f).forEach(sn=>{const{r:nr,f:nf}=sqRF(sn);const k=nr+','+nf;if(!visited.has(k)){visited.add(k);queue.push({r:nr,f:nf,moves:moves+1});}});}
  return null;
}
...
export function handleDojoClick(sn,r,f) {
  if(!dojoState.piece||!dojoState.from) return; sTick();
  const cur=dojoState.sel||sqName(dojoState.from.r,dojoState.from.f); const{r:cr,f:cf}=sqRF(cur);
  const bd=Array.from({length:8},()=>Array(8).fill(null)); bd[cr][cf]=dojoState.piece.code; const legal=getLegalMoves(bd,cr,cf);
...
export function updateDojoStats() { document.getElementById('dojo-moves').textContent=dojoState.moves; document.getElementById('dojo-best').textContent=dojoState.best!==null?dojoState.best:'?'; document.getElementById('dojo-done').textContent=dojoState.done+'/10'; }
