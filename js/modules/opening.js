...
import { getOpenings } from '../data_loader.js';
import { drawBoard } from '../board.js';

export let OP={ cat:'all',idx:0,answered:false };
let opBuilt=false, OPENINGS=[];

export async function initOpening() {
  OPENINGS=await getOpenings();
  if(!opBuilt){opBuilt=true;buildOpCats();} loadOpening();
}
...
export function buildOpCats() {
  ...
  [{id:'all',lbl:'All'},{id:'principles',lbl:'📐 Principles'},{id:'castling',lbl:'🏰 Castling'}].forEach(cat=>{
    ...
    b.onclick=()=>{ document.querySelectorAll('#s-opening .pcat').forEach(x=>x.classList.remove('on')); b.classList.add('on'); OP.cat=cat.id; OP.idx=0; loadOpening(); };
    nav.appendChild(b);
  });
}
...
export function loadOpening() {
  const list=getOpList(); if(!list.length) return; const op=list[OP.idx%list.length]; OP.answered=false;
  ...
  hideFB('op-fb'); document.getElementById('op-next').style.display='none'; buildOpChoice(op); say(op.q);
}
...
export function buildOpChoice(op) {
  ...
  [op.o1,op.o2].sort(()=>Math.random()-.5).forEach(opt=>{
    ...
    card.onclick=()=>{ if(OP.answered)return; OP.answered=true; sTick(); if(opt.ok){sOk();confetti(30);earn(2,'opening');showFB('op-fb','✅ '+opt.why,'ok');card.classList.add('ok');}else{sBad();showFB('op-fb','❌ '+opt.why,'no');card.classList.add('no');}; document.getElementById('op-next').style.display=''; say(opt.why); };
