...
import { getPatData } from '../data_loader.js';

export let PAT={ learned:new Set(),current:null,solSel:null,solSolved:false };
let PAT_DATA=[];

export async function initPatterns() {
  PAT_DATA=await getPatData();
  if(document.getElementById('pat-grid').children.length===0) buildPatGrid();
}

export function buildPatGrid() {
  const grid=document.getElementById('pat-grid'); if(!grid) return; grid.innerHTML='';
  PAT_DATA.forEach(pat=>{ const card=document.createElement('div'); card.className='pat-card'+(PAT.learned.has(pat.id)?' learned':''); card.innerHTML=`<div class="pi">${pat.icon}</div><div class="pn">${pat.name}</div><div class="pb2">${PAT.learned.has(pat.id)?'✅ Learned!':'Tap to learn'}</div>`; card.onclick=()=>openPat(pat); grid.appendChild(card); });
}

export function openPat(pat) {
  PAT.current=pat; PAT.solSel=null; PAT.solSolved=false;
  document.getElementById('pat-grid').classList.add('hidden'); document.getElementById('pat-lesson').classList.remove('hidden');
  ...
  hideFB('pat-fb'); document.getElementById('pat-next').style.display='none';
  ...
  drawBoard('pat-board',pat.fen,{click:(sn,r,f)=>handlePatClick(sn,r,f,pat)}); say(pat.explain);
}

export function handlePatClick(sn,r,f,pat) {
  if(PAT.solSolved) return; sTick(); const bd=parseFEN(pat.fen); const p=bd[r][f];
  if(!PAT.solSel){if(p&&p[0]==='w'){PAT.solSel=sn;const lm=getLegalMovesFiltered(bd,r,f);drawBoard('pat-board',pat.fen,{sel:sn,dot:lm,click:(s2,r2,f2)=>handlePatClick(s2,r2,f2,pat)});}return;}
  const solArr=Array.isArray(pat.sol)?pat.sol:[pat.sol]; const mv=PAT.solSel+sn;
  if(solArr.includes(mv)||solArr.includes(sn)){
    ...
    const nb=applyMove(bd,fr,ff,r,f); drawBoard('pat-board',nb,{g:[sn]});
    showFB('pat-fb','🏆 Checkmate! You know the '+pat.name+'! ✅','ok'); sWin(); confetti(60); earn(3,'patterns');
