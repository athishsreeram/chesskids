...
export function initLadder() { ldSetMode('learn') }

export function ldSetMode(mode) { 
    LD = { mode, step: 0, sel: null, done: false };
    ['learn', 'practice', 'challenge'].forEach(m => { 
        const btn = document.getElementById('ld-tab-' + m); if (btn) btn.classList.toggle('on', m === mode) 
    }); ldLoad() 
}

export function ldLoad() { 
    const modeData = LD_MODES[LD.mode]; if (!modeData) return; 
    const step = modeData.steps[LD.step]; if (!step) return; 
    LD.sel = null; LD.done = false; 
    document.getElementById('ld-task').textContent = step.task; 
    hideFB('ld-fb'); 
    document.getElementById('ld-next').style.display = 'none'; 
    ldBuildProgress(modeData.steps.length); 
    drawBoard('ld-board', step.fen, { click: (sn, r, f) => handleLadder(sn, r, f, step) }); 
    say(step.task) 
}
...
export function handleLadder(sn, r, f, step) { 
    ...
    if (!LD.sel) { 
        if (p && p[0] === 'w' && p[1] === 'R') { 
            LD.sel = sn; const lm = getLegalMovesFiltered(bd, r, f); 
            drawBoard('ld-board', step.fen, { sel: sn, dot: lm, click: (s2, r2, f2) => handleLadder(s2, r2, f2, step) }) 
        } 
        return 
    } 
    if (sn === LD.sel) { 
        LD.sel = null; 
        drawBoard('ld-board', step.fen, { click: (s2, r2, f2) => handleLadder(s2, r2, f2, step) }); return 
    } 
    const { r: fr, f: ff } = srf(LD.sel); 
    const legal = getLegalMovesFiltered(bd, fr, ff); 
    if (!legal.includes(sn)) { 
        showFB('ld-fb', 'Cannot! ' + step.hint, 'no'); sBad(); LD.sel = null; 
        drawBoard('ld-board', step.fen, { click: (s2, r2, f2) => handleLadder(s2, r2, f2, step) }); return 
    } 
    if (LD.sel === step.from && sn === step.to) { 
        LD.done = true; const nb = applyMove(bd, fr, ff, r, f); 
        drawBoard('ld-board', nb, { g: [sn] }); 
        showFB('ld-fb', '✅ ' + step.explain, 'ok'); 
        sOk(); earn(2, 'ladder'); 
        document.getElementById('ld-next').style.display = ''; say(step.explain); 
