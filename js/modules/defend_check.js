...
import { getDkConcepts,getDkScenarios } from '../data_loader.js';

export let DK={ skill:'move',idx:0,answered:false,hintUsed:0 };
let DK_CONCEPTS={}, DK_SCENARIOS={};

export async function initDefendCheck() {
  DK_CONCEPTS=await getDkConcepts(); DK_SCENARIOS=await getDkScenarios(); dkSetSkill('move');
}

export function dkSetSkill(skill) {
  DK.skill=skill; DK.idx=0; DK.answered=false; DK.hintUsed=0;
  ['move','block','capture','protect'].forEach(s=>{ const btn=document.getElementById('dk-tab-'+s); if(btn)btn.classList.toggle('on',s===skill); });
...
export function dkLoad() {
  const scenarios=DK_SCENARIOS[DK.skill]; const sc=scenarios[DK.idx%scenarios.length];
  ...
  document.getElementById('dk-kip').textContent=sc.task; hideFB('dk-fb'); document.getElementById('dk-next').style.display='none';
...
export function dkClick(sn,r,f,sc) {
  ...
  const correct=sc.answers&&sc.answers.includes(sn); const isBad=sc.bad&&sc.bad.includes(sn);
  if(correct){DK.answered=true;drawBoard('dk-board',sc.fen,{g:[sn]});showFB('dk-fb','🎉 '+sc.explain,'ok');sOk();confetti(40);earn(2,'defend');document.getElementById('dk-next').style.display='';say(sc.explain);}
  else if(isBad){showFB('dk-fb','❌ Dangerous! '+sc.hint,'no');sBad();drawBoard('dk-board',sc.fen,{r:[sn],click:(sn2,r2,f2)=>dkClick(sn2,r2,f2,sc)});}
